...
void DatabaseManager::createTables() {
    ...
    bool ok = query.exec(
        ...
        "instructions TEXT,"
        "imageUrl TEXT" // Проверь, что это тут есть!
        ")"
        );
    if (!ok) qDebug() << "Ошибка создания таблицы recipes:" << query.lastError().text();

    query.exec("CREATE TABLE IF NOT EXISTS favorites (recipe_title TEXT UNIQUE)");
    query.exec("CREATE TABLE IF NOT EXISTS history (id INTEGER PRIMARY KEY AUTOINCREMENT, recipe_title TEXT, viewed_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
    query.exec("CREATE TABLE IF NOT EXISTS shopping_list (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL)");
    query.exec("CREATE TABLE IF NOT EXISTS fridge_items (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE)");
}

void DatabaseManager::seedData() {
    QSqlQuery check("SELECT COUNT(*) FROM recipes");
    if (check.next() && check.value(0).toInt() == 0) {
        qDebug() << "Начинаю заполнение базы рецептами...";
        QSqlQuery query;
        // 11 колонок (включая id и imageUrl)
        query.prepare(
            "INSERT INTO recipes "
        ...
        auto add = [&](QString title, QString ing, QString tags, int cal, double p, double f, double c, int s, QString inst, QString img) {
            query.bindValue(":title", title);
            query.bindValue(":ing", ing);
            query.bindValue(":tags", tags);
            query.bindValue(":cal", cal);
            query.bindValue(":prot", p);
            query.bindValue(":fat", f);
            query.bindValue(":carb", c);
            query.bindValue(":serv", s);
            query.bindValue(":inst", inst);

            if (img.startsWith("http")) {
                query.bindValue(":img", img);
            } else {
                // В Qt 6 ПУТЬ ДОЛЖЕН БЫТЬ ТАКИМ (с RecipeWizard):
                query.bindValue(":img", "qrc:/RecipeWizard/qml/images/" + img.trimmed());
            }
...
bool DatabaseManager::addFavorite(const QString &title) {
    QSqlQuery query;
    query.prepare("INSERT OR IGNORE INTO favorites(recipe_title) VALUES(:title)");
    query.bindValue(":title", title);
    return query.exec();
}
...
bool DatabaseManager::removeFavorite(const QString &title) {
    QSqlQuery query;
    query.prepare("DELETE FROM favorites WHERE recipe_title = :title");
    query.bindValue(":title", title);
    return query.exec();
}
...
bool DatabaseManager::isFavorite(const QString &title) {
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM favorites WHERE recipe_title = :title");
    query.bindValue(":title", title);
...
QStringList DatabaseManager::getFavorites() {
    QStringList result;
    QSqlQuery query("SELECT recipe_title FROM favorites ORDER BY recipe_title");
    while (query.next()) {
...
void DatabaseManager::addToHistory(const QString &title) {
    QSqlQuery query;
    query.prepare("INSERT INTO history(recipe_title) VALUES(:title)");
    query.bindValue(":title", title);
    query.exec();
}
...
QStringList DatabaseManager::getHistory() {
    QStringList result;
    QSqlQuery query("SELECT recipe_title FROM history GROUP BY recipe_title ORDER BY MAX(viewed_at) DESC");
    while (query.next()) {
        result << query.value(0).toString();
    }
    return result;
...
bool DatabaseManager::addRecipe(const QString &title, const QString &ingredients, const QString &tags,
    ...
    QSqlQuery query;
    query.prepare(
        "INSERT INTO recipes (title, ingredients, tags, calories, proteins, fats, carbs, servings, instructions) "
        "VALUES (:title, :ingredients, :tags, :calories, :proteins, :fats, :carbs, :servings, :instructions)"
        );
    // ... остальной код (bindValue) оставляем как был ...
    query.bindValue(":title", title);
...
bool DatabaseManager::updateRecipe(int id, const QString &title, const QString &ingredients, const QString &tags,
                                   int calories, double proteins, double fats, double carbs,
                                   int servings, const QString &instructions) {
    QSqlQuery query;
    query.prepare(
        "UPDATE recipes SET "
        "title = :title, ingredients = :ingredients, tags = :tags, "
        "calories = :calories, proteins = :proteins, fats = :fats, "
        "carbs = :carbs, servings = :servings, instructions = :instructions "
        "WHERE id = :id"
        );
    query.bindValue(":id", id);
    query.bindValue(":title", title);
...
QStringList DatabaseManager::getAllTags()
{
    ...
    QSqlQuery query("SELECT tags FROM recipes");
    ...
    QStringList dynamicTags =
        uniqueTags.values();

    dynamicTags.sort();

    result.append(dynamicTags);
...
void DatabaseManager::addToShoppingList(const QString &ingredients) {
    // Если мы передаем строку через запятую (из рецепта)
    QStringList list = ingredients.split(",");
    QSqlQuery query;
    for (const QString &item : list) {
        QString clean = item.trimmed().toLower();
        if (!clean.isEmpty()) {
            // INSERT OR IGNORE для уникальности
            query.prepare("INSERT OR IGNORE INTO shopping_list (name) VALUES (:name)");
            query.bindValue(":name", clean);
            query.exec();
        }
    }
...
QStringList DatabaseManager::getShoppingList() {
    QStringList result;
    QSqlQuery query("SELECT name FROM shopping_list");
    while (query.next()) {
        result << query.value(0).toString();
    }
    return result;
...
void DatabaseManager::removeFromShoppingList(const QString &name) {
    QSqlQuery query;
    query.prepare("DELETE FROM shopping_list WHERE name = :name");
...
void DatabaseManager::clearShoppingList() {
    QSqlQuery("DELETE FROM shopping_list");
    emit shoppingListUpdated();
}
...
void DatabaseManager::addToFridge(const QString &name) {
    ...
    QSqlQuery query;
    query.prepare("INSERT OR IGNORE INTO fridge_items (name) VALUES (:name)");
    query.bindValue(":name", clean);
    query.exec();
    emit fridgeUpdated(); // Сигнал для QML
}

QStringList DatabaseManager::getFridgeList() {
    QStringList result;
    QSqlQuery query("SELECT name FROM fridge_items ORDER BY name");
...
void DatabaseManager::removeFromFridge(const QString &name) {
    QSqlQuery query;
    query.prepare("DELETE FROM fridge_items WHERE name = :name");
    query.bindValue(":name", name);
    if (query.exec()) emit fridgeUpdated();
}

void DatabaseManager::clearFridge() {
    QSqlQuery("DELETE FROM fridge_items");
    emit fridgeUpdated();
}
